jQuery(function ($) {
  ...
  function lockButton() {
    $payBtn().prop("disabled", !0).attr("aria-busy", "true");
  }
  function unlockButton() {
    $payBtn().prop("disabled", !1).removeAttr("aria-busy");
  }
  function stopDelay() {
  ...
  function closeSdk(e) {
    try {
      window.Cepta?.closeModal && window.Cepta.closeModal(e || "close");
    } catch {}
  }
  function fireOnce(e, t) {
    if (callbackFired) return;
    (callbackFired = !0), stopPolling(), closeSdk(e), broadcast(e, t);
  }
  ...
  function startPolling(e) {
    ...
    if (!window.Cepta?.confirmStatus)
      return void console.warn(
        "[Cepta] confirmStatus not available; skipping poll."
      );
    (userClosed = !1),
      (callbackFired = !1),
      (pollStarted = !0),
      ...
      (delayTimeoutId = setTimeout(function () {
        ...
        pollIntervalId = setInterval(async function () {
          if (userClosed) return void stopPolling();
          try {
            const t = await window.Cepta.confirmStatus(e);
            if (!t?.status) return;
            const n = t.data;
            if (!n) return;
            ...
            "Successful" === r
              ? fireOnce("success", e)
              : "Failed" === r && fireOnce("failed", e);
          } catch (t) {
            console.error("[Cepta] Polling error:", t),
              $("#wc-cepta-form").show(),
              fireOnce("close", e);
          }
        }, POLL_INTERVAL_MS);
  ...
  function onFailed(e) {
    console.error("Payment failed:", e), verifyTransaction(e);
  }
  function onClose(e) {
    (userClosed = !0), hideLoading(), unlockButton();
    try {
  ...
  async function handlePayment(e = { fromClick: !1 }) {
    ...
    activeRef = n;
    const o = getUrlRef();
    if (o) return void verifyTransaction(o);
    ...
    try {
      ...
      console.error("Cepta Checkout failed to launch:", e?.message),
        hideLoading(),
        unlockButton(),
        modal(
  ...
  window.addEventListener(
    ...
    $("#cepta-payment-button")
      ...
      .on("click", function (e) {
        return (
          e.preventDefault(), lockButton(), handlePayment({ fromClick: !0 }), !1
        );
      }),
    // $("#wc-cepta-form").hide(),
